        ...
        )) = second_replay_result
        {
                ...
                || !divergent_accounts.is_empty()
            {
                ...
                if cause.contains("JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4") {
                    ...
                    for (key, acc) in transaction_accounts_before_tx.iter() {
                        let mut file = std::fs::OpenOptions::new()
                            .create(true)
                            .append(true)
                            .open(format!("/home/sol/logs/{}_{:?}.log", tx.signature(), key))
                            .unwrap();
                        file.write(acc.data()).unwrap();
                    }
                    ...
                    for dm in [false, true] {
                        ...
                        let log_collector = match config.log_messages_bytes_limit {
                            ...
                            Some(log_messages_bytes_limit) => Some(
                                LogCollector::new_ref_with_limit(Some(log_messages_bytes_limit)),
                            ),
                        };
                        ...
                        *program_cache_for_tx_batch = program_cache_for_tx_batch_before_tx.clone();
                        ...
                        let mut execute_timings = execute_timings.clone();
                        let mut executed_units = 0u64;
                        let _ = invoke_context.process_message(
                            tx,
                            &mut execute_timings,
                            &mut executed_units,
                        );
                        invoke_context.iterate_vm_traces(
                            &|instruction_context, executable, register_trace| {
                                let mut file = std::fs::OpenOptions::new()
                                    .create(true)
                                    .append(true)
                                    .open(format!("/home/sol/logs/{}_{}.log", tx.signature(), dm))
                                    .unwrap();
                                writeln!(file, "{:?}", instruction_context.get_program_key())
                                    .unwrap();
                                let analysis =
                                    solana_sbpf::static_analysis::Analysis::from_executable(
                                        &executable,
                                    )
                                    .unwrap();
                                analysis
